...
const findInvalidCards = arr => {
  // Check nested array for invalid cc numbers
  // Return nested array
  // To Do
  let invalidArray = [];
...
const idInvalidCardCompanies = arr => {
  ...
  for(let i=0; i<arr.length; i++) {
    firstDigits.push(arr[i][0]);
  }
  console.log(firstDigits);

  uniqueDigits = firstDigits.filter((c, index) => {
    return firstDigits.indexOf(c) === index;
  })
  console.log(uniqueDigits);
  for(let i=0; i<uniqueDigits.length; i++) {
    if (uniqueDigits[i] === 3) {
      console.log("Amex");
    } else if (uniqueDigits[i] === 4) {
      console.log("Visa");
    } else if (uniqueDigits[i] === 5) {
      console.log("Mastercard");
    } else if (uniqueDigits[i] === 6) {
